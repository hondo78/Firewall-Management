...
**Connection settings are superadmin-only** (checks `is_superadmin`, not the `admin` perm): `firewall_out` nulls `api_url`, `api_username`, `has_api_password`, `api_key_expires_at`, `verify_tls` and `central_account_id` for everyone else and sends `may_edit_connection`. Creating a firewall, `/test` and `/diagnose` need a superadmin. With `firewall.manage`, `PUT /firewalls/{id}` may change only name and group: `_keep_connection` fills the connection fields from the DB and returns 403 on any attempted change.
