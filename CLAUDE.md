...
### Backups (`backups.py`, `routers/backups.py`, `worker._backup_due`)
- `config_backups` holds full configs **read live** via `connector.fetch_config` (gzip JSON in `data`; the run also refreshes the cache via `store_config`, so it detects drift too). Triggers are `scheduled` (worker) and `manual` (`POST /firewalls/{id}/backups`, needs `firewall.manage`). Deleting needs a superadmin.
- The schedule lives in settings `backup_*` (daily/weekly/monthly, local time from `TZ`). `last_slot`/`due_firewalls` back up each firewall once per slot. After a failure the worker waits 30 min, and `backup.failed` is audited and notified once per slot.
- Retention keeps the newest `backup_keep` unpinned backups per firewall. With `backup_to_directory` on, files also go to `BACKUP_DIR` (`./backups` volume) and are deleted with the row.
- Restore never writes to the firewall. `restore/review` diffs the backup against the cache (new/changed/removed, same token store as the importer), and `restore/apply` puts the selected ops into the user's draft (removals only when explicitly selected, new rules placed after their nearest surviving predecessor). The compare endpoint accepts `backup:<id>` as a ref.
- The SFOS built-in backup schedule (`/system/backup/settings`) is a **REST singleton**: `entities.REST_SINGLETONS`, fetched via `get_singleton` as one object named „Sicherungs-Zeitplan“ and changed only by `update` → `PATCH` without a name in the path (`_singleton_body` drops `name`). `changes._check_backup_settings` validates FTP/e-mail targets early. The FTP password is deliberately not handled, because it would end up in requests and audit. The REST API cannot download the .bak file itself.
...
**Connection settings are superadmin-only** (checks `is_superadmin`, not the `admin` perm): `firewall_out` nulls `api_url`, `api_username`, `has_api_password`, `api_key_expires_at`, `verify_tls` and `central_account_id` for everyone else and sends `may_edit_connection`. Creating a firewall, `/test` and `/diagnose` need a superadmin. With `firewall.manage`, `PUT /firewalls/{id}` may change only name and group: `_keep_connection` fills the connection fields from the DB and returns 403 on any attempted change. Central accounts (`routers/central.py`) likewise require `permissions.require_superadmin`. For the audit log, `permissions.redact` masks `SENSITIVE_AUDIT_KEYS` (api_url, client_id, tenant_id …) in the list, dashboard and CSV for non-superadmins, and full-text search skips the details of `firewall`/`central_account` entries for them. The stored entries stay untouched, so the hash chain is intact.
...
Format-aware: `components/entities.js` provides `isRestEntity`, `anyRuleView`, `anySummary`, `restRefOptions`, and `RestEditors.jsx` holds the REST object forms (addresses, groups, services, zones with device access, schedules, MAC, IPv6). SFOS-style rule UI lives in `SophosRules.jsx` (`SophosRuleEditor` with sections for source/destination, exclusions, users, web filter, security features, mail scan, QoS, heartbeat; `SophosNatEditor` with original/translated; `FeatureBadges`, `RuleDetails`, `natView` for the table and detail modal), policies in `SophosPolicies.jsx` (web/app/IPS/traffic-shaping forms plus `ObjectView` read views). The form is always the primary mode; JSON is the expert tab. Firewall and NAT rules (REST) follow the SFOS web UI: `pages/firewall/RuleTable.jsx` is the list („Regeln und Richtlinien“: Firewallregeln/NAT-Regeln and IPv4/IPv6 tabs, gear column picker, drag handle → position op, ⋮ `RowMenu`, bulk on/off). Clicking a name opens the editor as a **page** (`EditorShell page`, sticky save footer) instead of a modal. `SophosRuleEditor` mirrors „Edit firewall rule“ (`SfList` boxes with ⊖ and „Neues Element hinzufügen …“, `Check`, `Fold`); WAF rules get a reduced form, because the REST API only returns the `wafRule` reference. The REST API has no rule groups. Shared table parts (`Pager`/`usePaging`, `ColumnPicker`, `RowMenu`, `store`) live in `pages/firewall/tableParts.jsx`. `READ_ONLY_ENTITIES` (users, interfaces) are display-only, and the backend rejects ops on them. `EditorShell` switches its expert mode between JSON (REST) and XML. `pages/FirewallView.jsx` holds the studio view: `buildRows` overlays the user's draft on the cached config (preview comes from the backend `effective_config`). `components/Editors.jsx` has the form editors plus the XML expert mode (parsed server-side via `POST /api/xml/parse`). `components/entities.js` has the object helpers (list containers!). The `Picker` dropdown is an absolute overlay on purpose: an inline list shifted the layout on blur, and clicks missed.

### Languages (i18n)
The UI is German and English, and more languages can be added. **The key is the German source text**: `t('Firewall-Regel hinzufügen')` from `src/i18n/index.js` (no library). Dictionaries are `src/i18n/<code>.js`, registered in `LANGS`. The language is fixed at load time (localStorage `fwm.lang`, else the browser language). Switching reloads the page, so module-level constants with `t()` work too.
- **Every new UI string goes in `t()`, and its English entry goes in `en.js`.** `npm run i18n:check` must report „0 fehlen“.
