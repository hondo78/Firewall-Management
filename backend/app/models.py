...
class Firewall(Base):
    ...
    # Schreibweg für Konfigurationsänderungen: "central" (Import über Sophos Central) oder "xmlapi" (direkt)
    connector: Mapped[str] = mapped_column(String(20), default="xmlapi")
    central_account_id: Mapped[str | None] = mapped_column(ForeignKey("central_accounts.id", ondelete="SET NULL"),
                                                           nullable=True)
    central_id: Mapped[str] = mapped_column(String(100), default="")
    central_status: Mapped[dict] = mapped_column(JSON, default=dict)
    # Lokale XML-API: https://<host>:4444 (oder volle Basis-URL)
