  ...
  return (
    <>
      <div className="page-head">
        <h1>{t("Übersicht")}</h1>
        <span className="sub">{t("Hallo")} {me.display_name || me.username}</span>
      </div>
      {d.keys_expiring?.length > 0 && (
        <div className="alert warn">{t("API-Keys laufen bald ab oder sind abgelaufen:")} {d.keys_expiring.map((k) => (
          <Link key={k.id} to={`/firewalls/${k.id}/settings`} style={{ marginRight: 10 }}>{k.name} ({fmt(k.expires_at).split(',')[0]})</Link>))}
          {t("– neuen Key auf der Firewall erzeugen und in den Einstellungen eintragen.")}</div>
      )}
      <div className="grid tiles">
        <Link className="panel tile" to="/firewalls"><div className="num">{d.firewalls}</div><div className="lbl">{t("Firewalls")}</div></Link>
        <Link className="panel tile" to="/changes?tab=approve">
          ...
          <div className="lbl">{t("Warten auf Ihre Genehmigung")}</div>
        </Link>
        <Link className="panel tile" to="/changes?tab=open"><div className="num">{d.pending}</div><div className="lbl">{t("Offene Anträge")}</div></Link>
        <Link className="panel tile" to="/changes?tab=open"><div className="num">{d.approved}</div><div className="lbl">{t("Genehmigt, noch nicht ausgerollt")}</div></Link>
        <Link className="panel tile" to="/changes?tab=failed">
          <div className="num" style={{ color: d.failed ? 'var(--danger)' : undefined }}>{d.failed}</div>
          <div className="lbl">{t("Fehlgeschlagen / Konflikt")}</div>
        </Link>
        <Link className="panel tile" to="/firewalls">
          ...
          <div className="lbl">{t("Firewalls mit Sync-Fehler")}</div>
        </Link>
      </div>

      <div className="grid two" style={{ marginTop: 16 }}>
        <div className="panel">
          <div className="panel-head"><h3>{t("Zu genehmigen")}</h3><Link className="right small" to="/changes?tab=approve">{t("alle")}</Link></div>
          {!toApprove?.length ? <Empty>{t("Nichts zu tun.")}</Empty> : (
            <table><tbody>
              {toApprove.slice(0, 8).map((c) => (
                <tr key={c.id}>
                  <td className="nowrap"><Link to={`/changes/${c.id}`}>{crNo(c.number)}</Link></td>
                  <td>{c.title}<div className="small muted">{c.firewall} {t("· von")} {c.created_by}</div></td>
        ...
        <div className="panel">
          <div className="panel-head"><h3>{t("Meine Anträge")}</h3><Link className="right small" to="/changes?tab=mine">{t("alle")}</Link></div>
          {!mine?.length ? <Empty>{t("Noch keine Anträge.")}</Empty> : (
            <table><tbody>
              {mine.map((c) => (
                <tr key={c.id}>
                  <td className="nowrap"><Link to={`/changes/${c.id}`}>{crNo(c.number)}</Link></td>
        ...
        {problems.length > 0 && (
          <div className="panel">
            <div className="panel-head"><h3>{t("Firewalls mit Problemen")}</h3></div>
            <table><tbody>
              {problems.map((f) => (
                <tr key={f.id}>
                  ...
                  <td className="small">{f.last_sync_error
                    ...
                    : <span className="muted">{t("noch nie synchronisiert")}</span>}</td>
                </tr>
              ))}
            </tbody></table>
          </div>
        ...
        {canAnywhere(me, 'audit.view') && (
          <div className="panel">
            <div className="panel-head"><h3>{t("Letzte Aktivitäten")}</h3><Link className="right small" to="/audit">{t("Audit-Log")}</Link></div>
            <table><tbody>
              {d.recent.map((e) => (
                <tr key={e.id}>
